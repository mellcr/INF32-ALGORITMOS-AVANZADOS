...
int encontrarSoluciones(char tablero[N][M],int n,int m,int inicioX, int inicioY,int finX,int finY,char letra){
	int nuevoX, nuevoY;
	...
	for (int i=0; i<MAXMOV; i++){
		...
		if (evaluaMovimiento(tablero,n,m,nuevoX,nuevoY)){
			...
			if (encontrarSoluciones(tablero,n,m,nuevoX,nuevoY,finX,finY,letra)){
				return 1;
			}
...
int main(){
	...
	colocaMinas(tablero);
	tablero[0][0] = 'A';
	encontrarSoluciones(tablero,n,m,0,0,n-1,m-1,letra);
	return 0;
}
